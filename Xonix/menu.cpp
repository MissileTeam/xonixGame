...
menu::menu(float width, float hight)
{
	...
	mainmenu[1].setPosition(Vector2f(width / 2 - 50, hight / (Max_main_menu + 1) * 2));
	...
	mainmenu[2].setPosition(Vector2f(width / 2 - 50, hight / (Max_main_menu + 1) * 3));
...
void menu::moveDown()
{
	if (mainmenuSelected + 1 <= Max_main_menu) //check if not on the last item (exit)
	{
		...
		mainmenu[mainmenuSelected].setFillColor(Color::Blue); //change the new item's color
	}
}
